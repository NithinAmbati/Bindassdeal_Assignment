...
class Home extends Component {
  ...
  handleAdd = () => {
    ...
    const newData = {
      ...
      name,
      category,
      date: formatDate(new Date()), // Corrected from data to date
    };
  ...
  handleSave = (id) => {
    const updatedData = this.state.bindassdealData.map(
      (item) => (item.id === id ? { ...item, date: item.date } : item) // Ensure the date field is being updated
    );
    const { setItem } = useLocalStorage();
    setItem("bindassdeal_data", updatedData);
    this.getData();
                ...
                <td>
                  {isEditing && item.id === this.state.editingId ? (
                    <input
                      ...
                      value={item.date}
                      onChange={(e) =>
                        this.handleFieldChange(item.id, "date", e)
                      }
                    />
                  ) : (
                    item.date
